...
- Story: _As a `<role>`, I want `<capability>`, so that `<outcome>`._
...
_Backlog seeded from blueprint §4 (MVP scope) and the in-tree Medusa B2B modules (`company`, `approval`, `quote`, `employee`). Stories are deliberately small and testable; split any that grow beyond `M`._
